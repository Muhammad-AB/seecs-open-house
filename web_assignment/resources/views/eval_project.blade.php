...
<div class="container">
    ...
    <div class="card mb-3">
        <div class="card-body">
            ...
            <p class="card-text">Group Members: {{ $project->students[0]->user->name}} </p>
    ...
    @if(auth()->check())
        @if(auth()->user())
            ...
            <form method="POST" action="{{ route('mark.project', ['id' => $project->id]) }}">
                @csrf

                <div class="mb-3 bg-light">
                    <label for="projectMarks">Mark the Project (1 - 10):</label>
                    <input type="number" id="projectMarks" name="projectMarks" class="form-control" min="1" max="10" step="0.5" value='{{$project->marks}}' required>
                </div>
